"""Uncertainty-Aware Training Experiment (Self-Paced Curriculum Learning).

Trains a stochastic BNN model (MCD or VI) on one dataset with configurable
uncertainty-aware training strategies: soft weighting, hard curriculum, or both.

After a warmup phase, the model's own MC-dropout variance is used to:
  - Soft: Scale per-sample loss by exp(-T * u_i), down-weighting uncertain samples.
  - Curriculum: Keep only the lowest-uncertainty fraction, ramping from
    `curriculum_start` to `curriculum_end`.
  - Both: Apply soft weighting to the curriculum-filtered subset.

Each invocation creates one leaf directory:
  results/{experiment_name}/{dataset}/{unc_config}/seed_{seed}/

Usage examples:
    # Baseline (no uncertainty): warmup only, equivalent to standard training
    python experiments/exp_uncertainty_training.py --dataset METABRIC --unc-mode none --seed 0

    # Soft weighting with CRPS loss
    python experiments/exp_uncertainty_training.py --dataset METABRIC --unc-mode soft \
        --loss-type crps --temperature 2.0 --warmup-epochs 10 --seed 0

    # Hard curriculum with Cox loss
    python experiments/exp_uncertainty_training.py --dataset METABRIC --unc-mode curriculum \
        --loss-type cox --curriculum-start 0.55 --curriculum-end 1.0 --seed 0

    # Both soft + curriculum with joint loss
    python experiments/exp_uncertainty_training.py --dataset METABRIC --unc-mode both \
        --loss-type joint_crps --lambda-val 0.3 --temperature 2.0 --seed 0
"""
import sys
import os
...
from utility.loss import (CoxPHLoss, CRPSLoss, BrierScoreLoss,
                          ...
                          CRPSLossPerSample, BrierScoreLossPerSample,
                          JointCoxCalibrationLossPerSample)
from tools.uncertainty_trainer import UncertaintyTrainer
...
UNC_MODES = ["none", "soft", "curriculum", "both"]
LOSS_TYPES = ["cox", "ibs", "crps", "joint_ibs", "joint_crps", "joint_crps_kl"]
N_GRID_POINTS = 100
...
def make_unc_config_name(unc_mode, loss_type, lam, mu, temperature, warmup_epochs,
                         mc_passes, curriculum_start, curriculum_end):
    """Folder-safe name encoding all uncertainty + loss parameters."""
    parts = [f"unc_{unc_mode}", loss_type]
    ...
    if unc_mode != "none":
        ...
        parts.append(f"w{warmup_epochs}")
        parts.append(f"mc{mc_passes}")
    if unc_mode in ("curriculum", "both"):
        parts.append(f"cs{curriculum_start}")
        parts.append(f"ce{curriculum_end}")
    return "_".join(parts)
...
def build_loss_function(loss_type, lam, mu, t_train, e_train, per_sample=False):
    """Build the loss function. When per_sample=True and the uncertainty mode
    needs per-sample losses (soft or both), use the unreduced variants."""
    ...
    if loss_type == "cox":
        ...
        if per_sample:
            return BrierScoreLossPerSample(time_grid, km_times, km_probs), "IBS (per-sample)"
        return BrierScoreLoss(time_grid, km_times, km_probs), "Integrated Brier Score"
    elif loss_type == "crps":
        if per_sample:
            return CRPSLossPerSample(time_grid), "CRPS (per-sample)"
        return CRPSLoss(time_grid), "Right-censored CRPS"
    elif loss_type == "joint_ibs":
        km_times, km_probs = compute_censoring_km(t_train, e_train)
        if per_sample:
            cal = BrierScoreLossPerSample(time_grid, km_times, km_probs)
            return JointCoxCalibrationLossPerSample(cal, lam=lam), f"Joint Cox+IBS per-sample (λ={lam})"
        ...
        if per_sample:
            cal = CRPSLossPerSample(time_grid)
            return JointCoxCalibrationLossPerSample(cal, lam=lam), f"Joint Cox+CRPS per-sample (λ={lam})"
        ...
        if per_sample:
            cal = CRPSLossPerSample(time_grid)
            joint = JointCoxCalibrationLossPerSample(cal, lam=lam)
            print("  Note: marginal KL term not per-sample; falling back to reduced JointCoxCalibrationLoss.")
            cal2 = CRPSLoss(time_grid)
            return (JointCoxCalibrationLoss(cal2, lam=lam, marginal_loss=marg, mu=mu),
                    f"Joint Cox+CRPS+KL (λ={lam}, μ={mu}) [reduced]")
        cal = CRPSLoss(time_grid)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Uncertainty-aware training experiment (SPCL for survival analysis)",
    ...
    parser.add_argument("--model", default="mcd1", choices=ALL_MODELS,
                        help="Stochastic model (MCD or VI only)")
    ...
    # --- Loss args ---
    parser.add_argument("--loss-type", default="crps", choices=LOSS_TYPES,
                        help="Base loss function")
    parser.add_argument("--lambda-val", type=float, default=0.3,
                        help="Lambda for joint losses")
    parser.add_argument("--mu-val", type=float, default=0.0,
                        help="Mu for marginal calibration term")
    ...
    parser.add_argument("--temperature", type=float, default=2.0,
                        help="Exponential weighting strength: w_i = exp(-T * u_i)")
    parser.add_argument("--curriculum-start", type=float, default=0.55,
                        help="Fraction of data to keep at first post-warmup epoch")
    parser.add_argument("--curriculum-end", type=float, default=1.0,
                        help="Fraction of data at final epoch (1.0 = use all)")
    ...
    unc_config_name = make_unc_config_name(
        unc_mode, loss_type, lam, mu, args.temperature, args.warmup_epochs,
        args.mc_passes, args.curriculum_start, args.curriculum_end,
    )
    if args.experiment_name:
        experiment_name = args.experiment_name
    else:
        experiment_name = datetime.now().strftime("%Y%m%d") + "_uncertainty_training"

    experiment_dir = Path(pt.RESULTS_DIR) / experiment_name
    seed_dir = experiment_dir / dataset_name / unc_config_name / f"seed_{seed}"
    ...
    print(f"{'='*70}")
    print(f"Uncertainty-Aware Training Experiment (SPCL)")
    print(f"  Mode:        {unc_mode}")
    ...
    if unc_mode in ("curriculum", "both"):
        print(f"  Curriculum:  {args.curriculum_start:.2f} → {args.curriculum_end:.2f}")
    ...
    # ---- Build Loss Function ----
    need_per_sample = unc_mode in ("soft", "both") and loss_type != "cox"
    loss_function, loss_desc = build_loss_function(loss_type, lam, mu, t_train, e_train,
                                                   per_sample=need_per_sample)
    ...
    if model_name == "vi":
        ...
        raise ValueError(f"Uncertainty training requires stochastic model (MCD or VI), got: {model_name}")
    ...
    if trainer.uncertainty_history:
        unc_df = pd.DataFrame(trainer.uncertainty_history)
        unc_df.to_csv(seed_dir / "uncertainty_history.csv", index=False)
        print(f"  Uncertainty history: {seed_dir / 'uncertainty_history.csv'}")
    ...
    print(f"\n{'='*70}")
    print(f"RESULTS: {dataset_name} | {model_name} | {unc_mode} | {loss_desc} | seed={seed}")
    print(f"  CI:     {ci:.4f}")
    print(f"  IBS:    {ibs:.4f}")
    ...
    res_df["UncMode"] = unc_mode
    res_df["LossType"] = loss_type
    res_df["LossConfig"] = unc_config_name
    res_df["Lambda"] = lam
    res_df["Mu"] = mu
    res_df["Temperature"] = args.temperature
    res_df["WarmupEpochs"] = args.warmup_epochs
    res_df["McPasses"] = args.mc_passes
    res_df["CurriculumStart"] = args.curriculum_start
    res_df["CurriculumEnd"] = args.curriculum_end
    res_df["Seed"] = seed
    ...
    run_config = {
        ...
        "experiment_type": "uncertainty_training",
        "dataset": dataset_name, "model": model_name,
        "unc_mode": unc_mode,
        "loss_type": loss_type, "loss_desc": loss_desc,
        "loss_config": unc_config_name,
        "lambda": lam, "mu": mu,
        "temperature": args.temperature,
        "warmup_epochs": args.warmup_epochs,
        "mc_passes": args.mc_passes,
    ...
    index_row = {**metrics_dict, **{
        ...
        "UncMode": unc_mode,
        "LossType": loss_type, "LossConfig": unc_config_name,
        "Lambda": lam, "Mu": mu,
        "Temperature": args.temperature,
        "WarmupEpochs": args.warmup_epochs,
        "McPasses": args.mc_passes,
        "CurriculumStart": args.curriculum_start,
        "CurriculumEnd": args.curriculum_end,
        "Seed": seed,
